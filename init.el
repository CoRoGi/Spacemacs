...
This function should only modify configuration layer settings."
  (setq-default
   ...
   '(
     ...
     ;; better-defaults
     (colors
      :variables colors-colorize-identifiers 'all)
     emacs-lisp
     git
     helm
     kotlin
...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   dotspacemacs-themes '(doom-feather-dark
                         material
                         clues
                         alect-black
                         underwater
...
before packages are loaded."
  ...
  (add-to-list 'exec-path "/usr/local/bin/kotlinc")
  (add-hook 'org-mode-hook 'turn-on-auto-fill)
  (use-package ledger-mode
    :hook (ledger-mode . olivetti-mode))
  (add-to-list 'auto-mode-alist '("\\.dat\\'" . ledger-mode))
  (use-package olivetti
    :hook
    (org-mode . olivetti-mode)
    (kotlin-mode . olivetti-mode))
  (use-package lsp-mode
    :commands lsp
    :ensure t
    :hook (kotlin-mode . lsp)
    :init (add-to-list 'exec-path "/usr/local/bin/ktlint"))
                ...
                (" " "Agenda"
                 ((agenda "" nil)
                  ...
                  (tags-todo "-CANCELLED/!TODO"
                           ((org-agenda-overriding-header (concat "TODO Items"))
                            (org-tags-match-list-sublevels t)
                            (org-agenda-sorting-strategy
                             '(todo-state-down effort-up category-keep))))
                 nil)))))
  ...
  (add-hook 'after-init-hook 'org-excalidraw-initialize)
  (setq org-todo-keywords '((sequence "IDEA(i)" "TODO(t)" "NEXT(n)" "ACTIVE(a)" "WAITING(w)" "BACKLOG(b)" "|" "DONE(d)" "CANCELLED(c)")))
  (setq org-roam-capture-templates
        `(
          ...
          ("e" "exercise" plain
           "- [ ] %?" :prepend nil
           :target (file+head+olp "exercises.org"
                                  "#+title: Exercises"
                                  ("Exercises")))
          ("h" "Habits" plain
           "- [ ] %?" :prepend nil
           :target (file+head+olp "habits.org"
                                  "#+title: Habits"
                                  ("Habits")))
          ))
  (setq org-roam-dailies-capture-templates
    `(
      ("c" "capture" plain
       "%T %?\n%i\n-----------------------------------------------------------------------------------------\n" :prepend nil
       :target (file+head+olp "~/Personal/Org/Notes/capture.org"
                              "#+title: Capture Notes"
                              ("General")))
      ("d" "daily")
       ( "dl" "logs" plain "- %^U %?\n%i\n"
  ...
  (setq org-agenda-files '("~/Personal/Org/daily/"))
  ...
  (font-lock-add-keywords
   'org-mode '(("-----------------------------------------------------------------------------------------" . font-lock-warning-face)))
)
